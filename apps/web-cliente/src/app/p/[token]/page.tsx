...
export default function PortalPedidoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([i, c]) => { if (ok) { setInfo(i); setCatalogo(c) } })
  ...
  const add = useCallback((s: { id?: number; servicoId?: number; codigo?: string; nome: string; preco: number; desconto: number }, qtd = 1) => {
    ...
    setCart((prev) => {
      ...
      return {
        ...prev,
        [id]: {
          servicoId: id, codigo: s.codigo, nome: s.nome, preco: s.preco, desconto: s.desconto,
  ...
  function addPacote(p: Pacote) {
    p.itens.forEach((it) => add(it, it.quantidade))
  }
  ...
  if (erro && !info) {
    return (
      <div className="min-h-screen grid place-items-center p-6 text-center">
        <div>
          <p className="text-lg font-semibold text-slate-700">Link inválido</p>
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between gap-3">
          ...
          {info && info.saldo > 0 && (
            <div className="text-right">
              <p className="text-[11px] text-white/60">Crédito disponível</p>
              <p className="text-lg font-bold text-emerald-300">{brl(info.saldo)}</p>
            </div>
          )}
        </div>
      </header>
      ...
      {enviado ? (
        <div className="max-w-5xl mx-auto px-4 py-16 text-center">
          <div className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 text-3xl">✓</div>
          <h2 className="mt-4 text-xl font-bold text-slate-800">Pedido enviado!</h2>
          <p className="text-sm text-slate-500 mt-1">
            Nº <span className="font-mono font-semibold">{enviado.numero}</span> · total {brl(enviado.valorTotal)}
          </p>
          <p className="text-sm text-slate-500 mt-1">Seu pedido entrou na fila de recebimento do laboratório.</p>
          <button
            onClick={() => setEnviado(null)}
            className="mt-6 rounded-lg bg-histocell-600 hover:bg-histocell-700 text-white text-sm font-medium px-4 py-2"
          >
            Fazer novo pedido
          </button>
        </div>
      ) : (
        <div className="max-w-5xl mx-auto px-4 py-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px] items-start">
          {/* Catálogo / abas */}
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex border-b border-slate-200 overflow-x-auto">
              {([
                ...
                ['pacotes', 'Pacotes'],
                ['historico', 'Histórico'],
                ['populares', 'Mais usados'],
              ] as [Tab, string][]).map(([k, label]) => (
                <button
                  key={k}
                  onClick={() => setTab(k)}
                  className={`flex-1 whitespace-nowrap px-3 py-3 text-[13px] font-medium border-b-2 transition-colors ${
                    tab === k ? 'border-histocell-600 text-histocell-700' : 'border-transparent text-slate-500 hover:text-slate-700'
                  }`}
                >
                  {label}
                </button>
            ...
            <div className="p-4">
              {tab === 'catalogo' && (
                <div className="space-y-3">
                  <input
                    ...
                    className="w-full rounded-md border border-slate-200 px-3 py-2 text-[13px] focus:outline-none focus:ring-2 focus:ring-histocell-500"
                  />
                  ...
                  <div className="divide-y divide-slate-100 max-h-[55vh] overflow-y-auto">
                    {servicosFiltrados.slice(0, 200).map((s) => (
                      <ServicoRow key={s.id} s={s} onAdd={() => add(s)} />
                    ))}
                    {servicosFiltrados.length === 0 && (
                      <p className="py-8 text-center text-[13px] text-slate-400">Nenhum serviço encontrado.</p>
                    )}
                  </div>
                </div>
              )}
                    ...
                    catalogo!.pacotes.map((p) => (
                      <div key={p.id} className="rounded-lg border border-slate-200 p-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="text-[13px] font-bold text-slate-800 shrink-0">{brl(p.precoTotal)}</span>
                        </div>
              ...
              {tab === 'historico' && (
                <ListaServicos lista={catalogo?.historico ?? []} onAdd={add} vazio="Você ainda não tem histórico de serviços." />
              )}
              {tab === 'populares' && (
                <ListaServicos lista={catalogo?.populares ?? []} onAdd={add} vazio="Sem serviços em destaque." />
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 lg:sticky lg:top-4">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-slate-700">Seu pedido</h2>
              ...
              <div className="divide-y divide-slate-100 max-h-[45vh] overflow-y-auto">
                {itensCart.map((i) => (
                  <div key={i.servicoId} className="px-4 py-2.5">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-[13px] text-slate-700 leading-tight">{i.nome}</p>
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <div className="flex items-center gap-1">
                        <button onClick={() => setQtd(i.servicoId, i.quantidade - 1)} className="h-6 w-6 rounded border border-slate-200 text-slate-500">−</button>
                        <input
                          value={i.quantidade}
                          onChange={(e) => setQtd(i.servicoId, parseInt(e.target.value, 10) || 0)}
                          className="h-6 w-10 text-center rounded border border-slate-200 text-[12px]"
                        />
                        <button onClick={() => setQtd(i.servicoId, i.quantidade + 1)} className="h-6 w-6 rounded border border-slate-200 text-slate-500">+</button>
                      </div>
                      <span className="text-[13px] font-medium text-slate-700 tabular-nums">{brl(subtotal(i))}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="px-4 py-3 border-t border-slate-100 space-y-3">
              <textarea
                ...
                className="w-full rounded-md border border-slate-200 px-3 py-2 text-[13px] resize-none focus:outline-none focus:ring-2 focus:ring-histocell-500"
              />
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-500">Total</span>
                <span className="text-xl font-bold text-slate-900">{brl(total)}</span>
              </div>
              <button
                onClick={enviar}
                disabled={itensCart.length === 0 || enviando}
...
function CatPill({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${active ? 'bg-histocell-600 text-white' : 'bg-slate-100 text-slate-600'}`}
    >
      {children}
    </button>
  )
...
function ServicoRow({ s, onAdd }: { s: Servico; onAdd: () => void }) {
  return (
    <div className="flex items-center justify-between gap-3 py-2">
      <div className="min-w-0">
        <p className="text-[13px] text-slate-800 leading-tight truncate">
      ...
      <button onClick={onAdd} className="shrink-0 rounded-md bg-histocell-600 hover:bg-histocell-700 text-white text-xs font-medium px-3 py-1.5">
        Adicionar
      </button>
    </div>
  )
